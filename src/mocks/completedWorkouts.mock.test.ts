...
describe("completed workout mock history", () => {
  it("creates stable, typed catalog-backed activity across the previous 30 days", () => {
    ...
    const offsets = history.map((record) =>
      differenceInCalendarDays(
        startOfLocalDay(referenceDate),
        startOfLocalDay(new Date(record.completedAt)),
      ),
    );

    expect(history).toHaveLength(19);
    expect(ids.size).toBe(history.length);
    expect(history.every((record) => workoutIds.has(record.workoutId))).toBe(
      true,
    );
    ...
    expect(new Set(history.map((record) => record.durationMinutes))).toEqual(
      new Set([15, 20, 25, 30, 35, 40, 45, 55]),
    );
    expect(calculateCurrentStreak(history, referenceDate)).toBe(4);
    expect(
      aggregateActivityByDay(history, 7, referenceDate).reduce(
