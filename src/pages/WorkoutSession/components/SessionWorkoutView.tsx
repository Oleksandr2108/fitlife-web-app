...
function ExercisePreview({ exercise }: { exercise: Exercise }) {
  ...
  return (
    <div className="mt-6 rounded-control border border-border bg-background-secondary p-4">
      <p className="text-meta text-text-muted">Next</p>
      <div className="mt-1 flex items-center justify-between gap-4">
        <p className="font-semibold">{exercise.name}</p>
        <span className="shrink-0 text-sm text-text-secondary">{target}</span>
      </div>
    </div>
  );
}

export const SessionWorkoutView = memo(function SessionWorkoutView(
  props: SessionWorkoutViewProps,
) {
  const {
    exercise,
    nextExercise,
  ...
  if (phase === "rest") {
    return (
      ...
      >
        ...
        <div className="mt-8">
          <TimerDial
            totalSeconds={exercise.restDurationSeconds ?? 0}
            label={isPaused ? "Paused" : "Rest"}
          />
        </div>
      ...
      <div className="mt-8">
        {isTimed ? (
          <TimerDial
            ...
            label={isPaused ? "Paused" : "Remaining"}
          />
        ) : (
          <div className="mx-auto grid size-52 place-content-center rounded-full border-[10px] border-surface-muted text-center sm:size-60">
            <span className="text-6xl font-bold tabular-nums">
              {exercise.repetitions}
            </span>
            <span className="mt-2 text-xs font-semibold uppercase tracking-[0.16em] text-text-muted">
              Repetitions
            </span>
          </div>
        )}
