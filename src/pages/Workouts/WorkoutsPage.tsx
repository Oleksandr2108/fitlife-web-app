...
export function WorkoutsPage() {
  ...
  const workouts = data ?? []
  const filteredWorkouts = filterWorkouts(workouts, { search, category, difficulty })
  const hasActiveFilters = search.trim().length > 0 || category !== 'all' || difficulty !== 'all'
      ...
      <AppContainer>
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-base leading-7 text-text-secondary">
            Search focused sessions by goal, movement style, or experience level.
          </p>
        </div>
        ...
        {isError ? (
          <div className="mt-8 rounded-surface border border-border bg-surface p-6 text-center">
            <h2 className="text-card-title">We couldn’t load the workout library</h2>
          ...
          <>
            <div className="mt-8 flex min-h-11 items-center justify-between gap-4">
              <p className="text-sm text-text-secondary" aria-live="polite">
                {isPending ? 'Loading workouts…' : `${filteredWorkouts.length} ${filteredWorkouts.length === 1 ? 'workout' : 'workouts'}`}
              </p>
              {hasActiveFilters ? (
                <button
                  type="button"
            ...
            {isPending ? (
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-label="Loading workouts">
              ...
              <div className="rounded-surface border border-dashed border-border-strong bg-surface p-8 text-center sm:p-12">
                <Dumbbell aria-hidden="true" className="mx-auto size-9 text-accent" />
                <h2 className="text-card-title mt-4">No workouts match those filters</h2>
                <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-text-secondary">
                  Try a broader search or reset the filters to see the full library.
                </p>
