...
export function PlanProgress({ plan, completedWorkouts }: PlanProgressProps) {
  if (!plan) {
    return (
      <article className="rounded-surface border border-border bg-surface p-5 shadow-surface sm:p-6">
        <p className="text-card-title">Your Plan</p>
        <p className="mt-2 text-sm leading-6 text-text-secondary">
          Create a personalized 7-day routine and track every scheduled workout
          here.
        </p>
        <Link
          to="/onboarding"
  ...
  return (
    <article className="rounded-surface border border-border bg-surface p-5 shadow-surface sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            workouts completed · {completion.percentage}%
          </p>
        </div>
        <Link
          to="/plan"
      ...
      <ol
        className="mt-5 grid min-w-0 grid-cols-7 gap-1 sm:gap-1.5"
        aria-label="Personalized plan progress"
      >
        {plan.days.map((day) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`mx-auto mt-2 grid aspect-square w-full max-w-8 place-items-center rounded-full border ${completed ? "border-accent bg-accent text-accent-foreground" : day.isRestDay ? "border-border bg-background-secondary text-text-muted" : "border-border bg-surface-muted text-text-muted"}`}
                aria-label={`Day ${day.day}: ${day.isRestDay ? "rest day" : completed ? "completed" : "not completed"}`}
              >
                {completed ? (
                  <Check
                    aria-hidden="true"
