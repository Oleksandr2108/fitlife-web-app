        ...
        <div className="flex w-max min-w-full gap-2">
          {options.map((option) => (
            <button
              ...
              className={`min-h-11 shrink-0 whitespace-nowrap rounded-full border px-4 text-sm font-semibold transition-colors ${value === option.value ? "border-accent bg-accent text-accent-foreground" : "border-border bg-surface text-text-secondary hover:border-border-strong hover:text-text-primary"}`}
            >
              {option.label}
            </button>
          ))}
  ...
  return (
    <div className="grid min-w-0 max-w-full gap-4">
      <FilterRow
        label="Meal type"
        value={mealType}
        options={recipeMealOptions}
        onChange={onMealTypeChange}
      />
      <FilterRow
        label="Nutrition"
        value={characteristic}
        options={recipeCharacteristicOptions}
