...
const dimensions = {
  width: 360,
  height: 250,
  padding: { top: 18, right: 12, bottom: 34, left: 38 },
};
...
function points(
  ...
  return Array.from({ length }, (_, index) => ({
    ...
    label: String(index),
    fullLabel: `Day ${index}`,
    minutes: valueAt(index),
    workouts: valueAt(index) > 0 ? 1 : 0,
  }));
}
...
describe("activity chart geometry", () => {
  it.each([
    ...
    expect(geometry.coordinates.every(({ y }) => y <= geometry.baseline)).toBe(
      true,
    );
  });
});
