...
export function calculateTotalMinutes(records: CompletedWorkout[]): number {
  return uniqueRecords(records).reduce(
    (total, record) => total + Math.max(0, record.durationMinutes),
    0,
  );
}
...
export function calculateCurrentStreak(
  ...
  const activeDays = new Set(
    uniqueRecords(records).flatMap((record) => {
      const date = new Date(record.completedAt);
      return isValidDate(date) && date <= now ? [toLocalDateKey(date)] : [];
    }),
  );
...
export function calculateWeeklyCount(
  records: CompletedWorkout[],
  now = new Date(),
): number {
  ...
  return uniqueRecords(records).filter((record) => {
    const date = new Date(record.completedAt);
    return (
      isValidDate(date) && date >= weekStart && date < weekEnd && date <= now
    );
  }).length;
}
...
export function aggregateActivityByDay(
  ...
  uniqueRecords(records).forEach((record) => {
    const date = new Date(record.completedAt);
    if (!isValidDate(date) || (endOffset === 0 && date > now)) return;
...
export function compareActivityPeriods(
  ...
  const currentMinutes = current.reduce(
...
export function calculatePlanCompletion(
  ...
  const completedDays = new Set(
    uniqueRecords(records).flatMap((record) =>
      ...
      requiredDayNumbers.has(record.planDay)
        ? [record.planDay]
        : [],
    ),
  );
