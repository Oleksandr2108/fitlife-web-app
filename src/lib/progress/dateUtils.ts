...
export function formatRecentActivityDate(value: string, now = new Date()): string {
  const date = new Date(value)
  if (!isValidDate(date)) return 'Date unavailable'
  const difference = differenceInCalendarDays(startOfLocalDay(now), startOfLocalDay(date))
