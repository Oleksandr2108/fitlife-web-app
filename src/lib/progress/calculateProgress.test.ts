...
function record(
  ...
  return {
    ...
    completedAt: new Date(year, month, day, 10).toISOString(),
...
describe("progress calculations", () => {
  ...
  it("counts multiple workouts on one day but one streak day", () => {
    ...
    expect(aggregateActivityByDay(records, 7, now).at(-1)).toMatchObject({
      workouts: 2,
      minutes: 40,
    });
  });
  ...
  it("excludes future records from current week, streak, activity, and recent lists", () => {
    const future = record("future", 2026, 8, 9, 50);
    const today = record("today", 2026, 8, 8, 20);
    const records = [future, today];

    expect(calculateWeeklyCount(records, now)).toBe(1);
    expect(calculateCurrentStreak(records, now)).toBe(1);
    expect(
      aggregateActivityByDay(records, 7, now).reduce(
        (total, point) => total + point.minutes,
        0,
      ),
    ).toBe(20);
    expect(getRecentActivity(records, 5, now).map((item) => item.id)).toEqual([
      "today",
    ]);
  });

  it.each([7, 30] as const)(
    "returns all %i requested days including zero-activity days",
    (days) => {
      const points = aggregateActivityByDay(
        [record("1", 2026, 8, 8)],
        days,
        now,
      );
      expect(points).toHaveLength(days);
      expect(points.filter((point) => point.workouts === 0)).toHaveLength(
        days - 1,
      );
    },
  );
  ...
  it("sorts recent activity without mutating its source", () => {
    ...
    expect(getRecentActivity(source).map((item) => item.id)).toEqual([
      "new",
      "old",
    ]);
  ...
  it("calculates positive and negative period comparisons safely", () => {
    const current = aggregateActivityByDay(
      [record("current", 2026, 8, 8, 120)],
      7,
      now,
    );
    const previous100 = aggregateActivityByDay(
      [record("previous", 2026, 7, 31, 100)],
      7,
      now,
      -7,
    );
    const previous150 = aggregateActivityByDay(
      [record("previous", 2026, 7, 31, 150)],
      7,
      now,
      -7,
    );
    expect(compareActivityPeriods(current, previous100)).toEqual({
      kind: "increase",
      percentage: 20,
    });
    expect(compareActivityPeriods(current, previous150)).toEqual({
      kind: "decrease",
      percentage: 20,
    });
    expect(
      compareActivityPeriods(current, aggregateActivityByDay([], 7, now, -7)),
    ).toEqual({ kind: "new" });
  });
  ...
  it("matches plan completion by plan id and scheduled day", () => {
    ...
    expect(calculatePlanCompletion(plan, records)).toMatchObject({
      completed: 1,
      required: 2,
      percentage: 50,
    });
